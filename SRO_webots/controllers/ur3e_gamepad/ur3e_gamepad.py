...
GRIPPER_JOINT_CANDIDATES = [
    # Common Robotiq variants
    "robotiq_85_left_finger_joint",
    "robotiq_85_right_finger_joint",
    "left_finger_joint",
    "right_finger_joint",
    "gripper_finger_joint",
    # Your Webots names (exact, incl. spaces and ::)
...
class UR3eGamepadController:
    def __init__(self):
        ...
        # Acquire arm joints
        self.joints = []           # list[Motor]
        self.joint_limits = []     # list[(min,max)]
        self.joint_sensors = []    # list[PositionSensor]
        for name_list in POSSIBLE_JOINT_NAMES:
            ...
            # limits
            try:
                jmin = m.getMinPosition()
            except Exception:
                jmin = -math.pi
            try:
                jmax = m.getMaxPosition()
            except Exception:
                jmax =  math.pi
            if jmax < jmin:
                jmin, jmax = jmax, jmin
            self.joints.append(m)
            self.joint_limits.append((jmin, jmax))
            # sensor
            ps = m.getPositionSensor()
            if ps:
                ps.enable(self.timestep)
            self.joint_sensors.append(ps)
        for j in self.joints:
            j.setPosition(float('inf'))  # velocity control
            j.setVelocity(0.0)
    ...
    def step(self):
        ...
        # Guard against hitting hard joint limits: stop outward motion near bounds
        eps = 0.01
        for i in range(6):
            # read current position if sensor available
            pos = None
            if self.joint_sensors[i]:
                pos = self.joint_sensors[i].getValue()
            jmin, jmax = self.joint_limits[i]
            v_cmd = target[i]
            if pos is not None:
                if pos >= (jmax - eps) and v_cmd > 0:
                    v_cmd = 0.0
                if pos <= (jmin + eps) and v_cmd < 0:
                    v_cmd = 0.0
            # filter and apply
            self.filtered_cmd[i] = smooth(self.filtered_cmd[i], v_cmd)
            try:
